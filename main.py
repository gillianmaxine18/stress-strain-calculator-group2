...
class Material:
    ...
    def can_withstand_stress(self, stress: float) -> bool:
        # yield_strength is in MPa, stress is in Pa
...
class StressStrainTest:
    ...
    def youngs_modulus(self) -> float:
        if self.strain == 0:
            return 0
        # Stress (Pa) / Strain -> converted to GPa by dividing by 1e9
    ...
    def factor_of_safety(self) -> float:
        if self.stress <= 0:
            return float('inf')
        # Convert yield_strength (MPa) to Pa before dividing by stress (Pa)
...
class TestAnalysisSystem:
    ...
    def display_calculation_history(self):
        ...
        for i, test in enumerate(self.tests, start=1):
            ...
            # Fixed attribute names from applied_force -> _force and area -> _area
